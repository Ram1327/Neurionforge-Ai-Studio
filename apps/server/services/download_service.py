...
class DownloadService:
    ...
    async def enqueue_download(self, repo_id: str, filename: str) -> str:
        ...
        if self.active_job and self.active_job.repo_id == repo_id and self.active_job.filename == filename:
    ...
    def _execute_download_sync(self, state: DownloadState):
        """Synchronous download execution with HTTP range resumption"""
        url = f"https://huggingface.co/{state.repo_id}/resolve/main/{state.filename}"
                    ...
                    for chunk in resp.iter_content(chunk_size=512 * 1024):
                        ...
                        if chunk:
                            ...
                            if now - last_update_time >= 0.4:
                                ...
                                if total_bytes > 0:
                                    state.percent = (state.bytes_downloaded / total_bytes) * 100
                                    remaining_bytes = total_bytes - state.bytes_downloaded
